...
const opcua::DataType &getPointDataType()
{
    static const opcua::DataType dt =
        opcua::DataTypeBuilder<Point>::createStructure("Point", {1, 1}, {1, 4042 })
...
{
    static const opcua::DataType dt =
        opcua::DataTypeBuilder<Opt>::createStructure("Opt", {1, 3}, {1, 4644})
...
namespace opcua {
    ...
    template <>
    struct TypeConverter<Measurements>
    {
        ...
        static void toNative(const ValueType &src, NativeType &dst)
        {
            ...
            dst.encoding = UA_EXTENSIONOBJECT_ENCODED_BYTESTRING;
            // Set Type ID (if necessary, example given below)
            dst.content.encoded.typeId = UA_NODEID_NUMERIC(MeasurementsNodeIdInfo.namespaceIndex, MeasurementsNodeIdInfo.identifier);
        }
    };
    
    template <>
    struct TypeConverter<Opt>
    {
        using ValueType = Opt;
        using NativeType = UA_ExtensionObject;
        using ValidTypes = TypeIndexList<UA_TYPES_EXTENSIONOBJECT>;
